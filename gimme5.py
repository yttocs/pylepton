	...
	if M['m00'] != 0:
	  ...
	  cx = int(M['m10']/1)
        ...
        for xy in cnt:
            x = xy[0][0]
            y = xy[0][1]
            dist = 10.0 / (math.hypot(cx - x, cy - y) + 0.001)
            dist_map[dist] = xy
        ...
        if len(anchors_map) == 3:
           ...
           if text == "Y":
              ...
              msg = str(avg)
              cv2.putText(img, msg, (2,58), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.5, 255)
        break
...
def main(flip_v = False, alpha = 128, device = "/dev/spidev0.0", text = "N"):
...
if __name__ == '__main__':
  ...
  parser.add_option("-t", "--text",
                    dest="text", default="N",
                    help="specify show text or not")

  (options, args) = parser.parse_args()

  main(flip_v = options.flip_v, alpha = options.alpha, device = options.device , text = options.text )
